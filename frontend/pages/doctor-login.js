...
export default function DoctorLoginPage() {
  ...
  return (
    <>
      <Head>
        <title>Doctor Login - HealthGuard</title>
        <meta name="description" content="Doctor login to HealthGuard patient care portal" />
      </Head>
        ...
        <div style={styles.card}>
          ...
          <form onSubmit={handleLogin} style={styles.form}>
            <div style={styles.field}>
              ...
              <input id="doctor-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required placeholder="doctor@hospital.com" style={styles.input} />
            </div>
            <div style={styles.field}>
              <label style={styles.label}>Password</label>
              <div style={{ position: 'relative' }}>
                <input id="doctor-password" type={showPw ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} required placeholder="••••••••" style={{ ...styles.input, width: '100%', paddingRight: '44px', boxSizing: 'border-box' }} />
            ...
            <button type="submit" id="doctor-login-btn" disabled={loading} style={styles.button}>
              {loading ? 'Logging in...' : 'Doctor Login'}
            </button>
          </form>
          <div style={styles.links}>
            <a href="/doctor-signup" style={styles.link}>New doctor? Register here</a>
          </div>
        </div>
      </div>
    </>
...
const styles = {
  container: { minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 50%, #0f172a 100%)', fontFamily: "'Segoe UI', sans-serif" },
  card: { background: 'rgba(30,41,59,0.9)', border: '1px solid rgba(148,163,184,0.1)', borderRadius: '16px', padding: '40px', width: '100%', maxWidth: '420px', boxShadow: '0 25px 50px rgba(0,0,0,0.5)' },
